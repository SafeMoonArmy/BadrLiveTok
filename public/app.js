...
function connect() {
    ...
    const validSerials = ['Badrtik2023Vip', 'Badvip2023', 'AnotherSerial']; // Ajoutez autant de numéros de série valides que nécessaire

    if (!validSerials.includes(enteredSerial)) {
        alert('Numéro de série invalide. Veuillez entrer un numéro de série valide pour vous connecter.');
        return;
    }
    ...
    if (uniqueId !== '') {
        ...
        connection.connect(uniqueId, {
            ...
            // Masquer le champ de saisie du numéro de série après la connexion
            $('#serialInputContainer').hide();

        }).catch(errorMessage => {
            $('#stateText').text(errorMessage);

            // Programmer la prochaine tentative si le nom d'utilisateur OBS est défini
